...
export default function HomePage() {
  ...
  return <><Header /><main id="top">
    ...
    <section id="visit" className="section visit-section"><div className="shell"><div className="visit-card"><div className="visit-details"><p className="eyebrow">Make DeMarco's your stop today</p><h2>Tonight's dinner is one easy stop away.</h2><p>{status.label}. Tap directions and we'll route you straight to the store.</p><div className="visit-list"><a href={store.directionsUrl} target="_blank" rel="noreferrer"><MapPin /><span><small>Address</small>{store.address}</span></a><a href={store.phoneHref}><Phone /><span><small>Phone</small>{store.phoneDisplay}</span></a><div><Clock3 /><span><small>Hours</small>Mon–Sat: 10 AM–8 PM<br />Sunday: 11 AM–4 PM</span></div></div><div className="visit-actions"><a className="button" href={store.directionsUrl} target="_blank" rel="noreferrer"><Navigation size={19} /> Start Directions — Visit Today</a><a className="social" href={store.instagram} target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram /></a><a className="social" href={store.facebook} target="_blank" rel="noreferrer" aria-label="Facebook"><Facebook /></a></div></div><div className="map-wrap"><iframe title="Map to DeMarco's Fine Foods" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q=DeMarco%27s+Fine+Foods,+1349+Grand+Marais+Rd+W,+Windsor,+ON&output=embed" /></div></div></div></section>
