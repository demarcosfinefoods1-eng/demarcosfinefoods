...
export default function HomePage() {
  ...
  return <><Header /><main id="top">
    ...
    <section className="trust-strip" aria-label="Store highlights"><div className="shell trust-grid"><div><MapPin />Easy Windsor neighbourhood stop</div><div><Clock3 />Open 7 days a week</div><div><ShoppingBasket />Fresh food, beer & wine together</div></div></section>
    ...
    <section id="visit" className="section visit-section"><div className="shell"><div className="visit-card"><div className="visit-details"><p className="eyebrow">Stop in on your way home</p><h2>Take the easy route to tonight's dinner.</h2><div className="visit-list"><a href={store.directionsUrl} target="_blank" rel="noreferrer"><MapPin /><span><small>Address</small>{store.address}</span></a><a href={store.phoneHref}><Phone /><span><small>Phone</small>{store.phoneDisplay}</span></a><div><Clock3 /><span><small>Hours</small>Mon–Sat: 10 AM–8 PM<br />Sunday: 11 AM–4 PM</span></div></div><div className="visit-actions"><a className="button" href={store.directionsUrl} target="_blank" rel="noreferrer"><Navigation size={19} /> Get Directions — Stop In</a><a className="social" href={store.instagram} target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram /></a><a className="social" href={store.facebook} target="_blank" rel="noreferrer" aria-label="Facebook"><Facebook /></a></div></div><div className="map-wrap"><iframe title="Map to DeMarco's Fine Foods" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q=DeMarco%27s+Fine+Foods,+1349+Grand+Marais+Rd+W,+Windsor,+ON&output=embed" /></div></div></div></section>
